...
class PathNavigationViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        var arr = [Any]() as! [MapboxDirections.Waypoint]
        
        for waypoint in (path?.mapItemWaypoints)! {
            arr.append(MapboxDirections.Waypoint(coordinate: CLLocationCoordinate2D(latitude: waypoint.placemark.coordinate.latitude, longitude: waypoint.placemark.coordinate.longitude), name: "Mapbox"))
        }
        
        
        let options = RouteOptions(waypoints: arr as! [MapboxDirections.Waypoint])//, profileIdentifier: .automobileAvoidsTraffic)
